...
function aj()
{
	"use strict";
	$.ajax({
		url: "assets/infolayout.json",
		dataType: "json",
		success: function(m)
		{
			jjson=eval(m).infoText;
			console.log("ajax success at "+this.url);
		},
		complete: function()
		{
			jjson.index =function()
			{
				var tmp = new Array(jjson.length);
				for(var i=0;i<jjson.length;i++){tmp[i]=[ jjson[i].id,i ];}
				tmp.sort(function(a,b){return (a[0]===b[0])?(a[1]-b[1]):(a[0]<b[0]?(-1):1);});
				return tmp;
			}();
			jjson.find = function(str)
			{
				var l=0,r=jjson.length-1,flag=false;
				var m,res;
				while(l!==r)
				{
					m=Math.floor((l+r)/2);
					console.log("L"+l+"M"+m+"R"+r);
					if(str===jjson.index[m][0]){flag=true;res=m;break;}
					else if(str<jjson.index[m][0]){r=m;}
					else if(str===jjson.index[r][0]){flag=true;res=r;break;}
					else{l=m;}
				}
				return flag?jjson.index[res][1]:(-1);
			};
			console.log(jjson);
    	}
	});
...
function showinfostr(str)
{
	"use strict";
	var para=document.getElementById("infoshowHere");
	var ix=jjson.find(str);
	if(ix==="-1"){para.innerHTML="no content of "+str;}
	else { para.innerHTML="<strong>"+jjson[ix].name+"</strong><br>"+jjson[ix].text;}
}
